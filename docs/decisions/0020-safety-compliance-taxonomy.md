...
Columns: `check`, `category`, `pipeline stage(s)`, `severity`, `blocks?`, `example copy`, `detection status` (implemented/planned), `owning reference`. One row per conceptual check family (e.g. all archive-extraction failure modes are one row), not one row per distinct error string — this is a policy record, not an error-message catalog.
...
| 7 | Workflow → skill reference missing (single package) | Risky | Publish, Inspect | Warning | No | "workflow %q references skill %q, which was not found in this package" | Implemented (severity classified by this ADR; previously an unweighted `Note`) | `internal/packages/validate.go` |
...
| 10 | Declared filesystem-read capability — broad | Risky | Publish, Inspect, Enable, Pull/Import | Warning | No | "declares filesystem read access outside its own package (`~`, `*`, `/`)" | Planned (breadth rule defined by this ADR) | This ADR; ADR 0006 |
| 11 | Declared filesystem-read capability — scoped | Informational | Inspect | Info | No | "declares read access confined to its own content" | Planned | This ADR; ADR 0006 |
| 12 | Declared network capability (any) | Risky | Publish, Inspect, Enable, Pull/Import | Warning | No | "declares network access" | Implemented display; warning framing new | `internal/packages/manifest.go`; ADR 0006 |
...
| 15 | No declared provider compatibility | Informational | Inspect | Info | No | "package declares no entrypoints for claude or codex" | Implemented display | `internal/packages/{manifest,portability}.go` |
| 16 | Setup/materialization write confirmation | Procedural | Enable/Materialize | Confirmation-gated | Blocks silent write | "will create X — continue?" (also lists Risky/Sensitive findings) | Implemented; UX folds in taxonomy warnings per this ADR and ADR 0019 | `internal/materialize/materialize.go`; ADR 0008, 0019 |
...
- Two rows (7, and the filesystem/network capability rows) get a severity reclassification without any code change: row 7 moves from an unweighted `Note` to an explicit Risky-tier warning in messaging; capability display gains explicit Risky/Informational framing where today it's undifferentiated printed text. Implementing that framing (deciding exactly where in `validate`/`inspect` output the distinction shows up) is separate follow-up work, not part of this ADR.
...
- **Implement PII detection** (rows 2, 13): a documented pattern list, split into a Risky-tier high-risk-pattern check (SSN/credit-card/gov-ID shapes) and a Sensitive-tier general-contact-info check (name/email/phone/address), following the same "grow the list, don't build an engine" philosophy as 0009.
- **Implement capability-breadth classification** (rows 10, 11): apply the syntactic broad-vs-scoped rule above to `Filesystem.Read` at inspect/validate time; today capabilities are printed without this distinction.
- **Surface the row-7 and capability-severity reclassifications in CLI/registry output** — this ADR settles the policy, not the exact rendering; a follow-up should update `runPackageValidate`/`runInspect` output to reflect Risky vs. Informational framing where it's currently flat text.
